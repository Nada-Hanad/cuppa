	...
	const handleButtonClick = (buttonType) => {
	  ...
	  switch (buttonType) {
		...
		case 'Not-Pending':
		  ...
		  default:
			setReclamations(
			  defaultData.filter((d) => d.id_reclamation !== null)
			);
		  break;
	  }
	};
	...
	const tableHooks = (hooks) => {
		hooks.visibleColumns.push((columns) => [
			...columns,
			{
				...
				Cell: ({ row }) => {
					return (
						<button className='w-[100px] h-[50px] rounded-full bg-[#343A49] text-white flex items-center justify-center' onClick={() => {
							// Ajoutez ici la logique pour rediriger vers la page "add.js"
							// par exemple : router.push('/add');
							router.push(`/ac/detailsReclamations/${row.original.id_reclamation}`);
							}}>
							Détails
						  </button>
	...
	return (
      ...
      >
        Tous
				...
				<button
        className={`w-[180px] h-[60px] rounded-[15px] border-[3px] border-[#343A49] text-[#343A49] font-semibold text-[20px] flex items-center justify-evenly ${
          selectedButton === 'Quality' ? 'bg-[#343A49] text-white' : 'bg-white'
        }`}
      ...
      <button
        className={`w-[180px] h-[60px] rounded-[15px] border-[3px] border-[#343A49] text-[#343A49] font-semibold text-[20px] flex items-center justify-evenly ${
          selectedButton === 'Delivery' ? 'bg-[#343A49] text-white' : 'bg-white'
        }`}
      ...
      <button
        className={`w-[180px] h-[60px] rounded-[15px] border-[3px] border-[#343A49] text-[#343A49] font-semibold text-[20px] flex items-center justify-evenly ${
          selectedButton === 'Pending' ? 'bg-[#343A49] text-white' : 'bg-white'
        }`}
      ...
      <button
        className={`w-[180px] h-[60px] rounded-[15px] border-[3px] border-[#343A49] text-[#343A49] font-semibold text-[20px] flex items-center justify-evenly ${
          selectedButton === 'Not-Pending' ? 'bg-[#343A49] text-white' : 'bg-white'
        }`}
