...
export default function Profil() {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await axios
      .post(
        `${DEPLOY_URL}/api/account.management/modifyAccount/${profil?.role?.libelle_role}/${profil?.id_utilisateur}`,
        data
      )
  ...
  const fetchProfile = async () => {
    ...
    const response = await axios
      .get(`${DEPLOY_URL}/api/account.management/getProfileWithClient/`, config)
      .catch((e) => console.log(e));
    if (response) {
      ...
      <div className="flex flex-col items-center justify-center md:flex-row gap-x-20 ">
        ...
        <div className="flex flex-col items-center justify-center w-4/12">
          <button
            ...
            className="self-end px-4 py-4 mx-auto mb-8 text-white bg-dark-grey rounded-xl"
          >
            {" "}
            Modifier Profil
          </button>
